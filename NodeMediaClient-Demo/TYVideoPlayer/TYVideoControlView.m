...
{
    [_delegate videoControlView:self recieveControlEvent:TYVideoControlEventRefresh];
    
    if(!_contentView.hidden) {
        [self performSelector:@selector(refreshMediaControl) withObject:nil afterDelay:0.2];
    }
}
...
- (void)layoutSubviews
{
    ...
    _loadingView.center = _contentView.center;
}
